...
import { deleteImages, deleteItem, getImages } from "../service/itemService";
...
export default function Dropdown({ _id, onItemDeleted, onItemSell }: DropdownProps) {
    ...
    const handelSell = async () => {
        const response: CardDetailsProps = await getImages(_id)
        deleteImages(response.images!);
        setOpen(false);
        onItemSell();
    }
            ...
            {open && (
                <div className="absolute right-0 mt-2 w-40 bg-bg border border-header rounded-lg shadow-lg z-50">
                    <ul className="py-2 text-sm text-gray-700">
                        ...
                        <li className="px-4 py-2 hover:bg-header hover:text-text cursor-pointer rounded m-1" onClick={handelSell}>
                            مباع
                        </li>
                    </ul>
                </div>
