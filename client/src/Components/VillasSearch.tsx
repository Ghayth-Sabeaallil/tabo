    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if ([...queryParams].length == 0) {
                ...
                const itemsData = getByFilter(city, "فيلا", Number(room), Number(price + "000000"), Number(area));
                setItems(await itemsData);
            }
            if ((await getVillas()).length >= 0) setLoading(false);
        };
        fetchData();
    }, [location]);
